...
class MySQLConnection(object):
	def __init__(self, app, db):
		# we got db as an argument
		# change the port to match the port your SQL server is running on
		config = {'host': 'localhost', 'database': db, 'user': 'root', 'password': 'root', 'port': '8889'}

        # this will use the above values to generate the path to connect to your sql database
		DATABASE_URI = "mysql://{}:{}@127.0.0.1:{}/{}".format(config['user'], config['password'], config['port'], confit['database'])
